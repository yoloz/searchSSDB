 ...
 * <p>
 * lucene.experimental
 * {@link org.apache.lucene.search.SearcherLifetimeManager}
 */
...
public class SearcherLifetimeManager implements Closeable {
    ...
    private static class SearcherTracker implements Comparable<SearcherTracker>, Closeable {
        ...
        private final double recordTimeSec;
        private final long version;

        private SearcherTracker(IndexSearcher searcher) {
    ...
    final static class PruneByAge {
         ...
         **/
        private boolean doPrune(double ageSec) {
    ...
    /**
     * Calls provided {@link PruneByAge} to prune entries.
     *
     * <p><b>NOTE</b>: you must peridiocally call this, ideally
     * from the same background thread that opens new
     * searchers.
     */
    synchronized void prune(PruneByAge pruner) throws IOException {
...
//        Collections.sort(trackers);
        ...
        for (SearcherTracker tracker : trackers) {
            final double ageSec = now - tracker.recordTimeSec;
            if (pruner.doPrune(ageSec)) {
